  ...
  init.atom = () => {
    ...
    ;(atom && atom.plugins.length > 0)
      ? atom.plugins.forEach((plugin, idx, arr) => {
          console.info(chalk.green(` --> Installing plugin ${plugin}...`))
          exec(`apm install ${plugin}`)
        })
      : console.info(
        chalk.gray(' --> No plugins declared, skipping this process.'))
    ...
    ;(atom && atom.config.length > 0)
      ? atom.config.forEach((config, idx, arr) => {
          ...
          sh.cp('-f', `${config.source}`, `${home}/${config.target}`)
        })
      : console.info(
        chalk.gray(' --> No configs declared, skipping this process.'))
  }
